...
file_to_read = open(f'{path}\\{list_dir[-2]}', "rb")
...
column2 = 'Flow'
print("z_dataset : ", dataset["z_dataset"])
# clustering_df = pd.DataFrame(dataset["df"], columns=[f'{column1}'])
clustering_df = pd.DataFrame(dataset["z_dataset"], columns=[f'{column1}'])
...
plt.scatter(d_numpy[y_pred == 2, 0], d_numpy[y_pred == 2, 1], s=25, c='black', label='Cluster 3')
